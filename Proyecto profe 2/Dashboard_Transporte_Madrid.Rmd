...
### Centralidad Comparada {data-commentary-width=400}
...
centralidad_comparada <- data.frame(
  ...
  Valor = c(mean(degree(g_bus)), max(degree(g_bus)), edge_density(g_bus),
            mean(degree(g_metro)), max(degree(g_metro)), edge_density(g_metro),
            mean(degree(g_bicimad)), max(degree(g_bicimad)), edge_density(g_bicimad),
            mean(degree(g_parking)), max(degree(g_parking)), edge_density(g_parking))
)

plot_ly(centralidad_comparada, x = ~Metrica, y = ~Valor, color = ~Capa, colors = c("#e74c3c", "#2c7bb6", "#27ae60", "#f39c12"), type = 'bar') %>%
  layout(title = "Comparación de Centralidad", barmode = 'group', legend = list(orientation = 'h'))
```
...
Se observa una drástica variación en la topología: el Autobús posee máxima densidad local por su proximidad, mientras que el Metro optimiza eficiencia con conexiones estratégicas. Esto valida numéricamente que cada modo tiene una estructura de red adaptada a su función específica (velocidad vs proximidad).

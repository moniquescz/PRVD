...
if (requireNamespace("rgl", quietly = TRUE)) {
  ...
  plot_multiplex3D_fix(
    ...
    layer.shift.y = 0,
    layer.space = 10,    # Espacio vertical generoso relativo a la escala normalizada
  )
...
cat("DEBUG: Iniciando preparación de datos...\n")
...
cat("DEBUG: Aristas preparadas. Total:", nrow(edges_full), "\n")
```
...
cat("DEBUG: Construyendo Supra-Matriz manualmente ( Dim:", TotalNodes, "x", TotalNodes, ")\n")
...
cat("DEBUG: Calculando MultiPageRank (Optimizado con igraph)...\n")
...
## 2. Versatilidad y Ranking Multicapa (con muxViz)
...
Para la realización de este estudio, la librería `muxViz` en R ha sido la herramienta fundamental para el análisis matemático riguroso. El proceso se ha estructurado en fases clave:

1.  **Modelado de la Red**: Hemos transformado los datos espaciales a "Extended Edge Lists" y utilizado la función `BuildSupraAdjacencyMatrixFromExtendedEdgelist()` de muxViz. Esto generó el objeto tensorial (Supra-Matriz) que unifica matemáticamente las capas de Metro, Bus, BiciMAD y Parking.
...
2.  **Análisis de Métricas**: Para cuantificar la relevancia real de las paradas, hemos calculado:
    ...
    *   **PageRank Multicapa**: Mediante `GetMultiPageRankCentrality()`. Esta métrica es superior a la centralidad clásica, ya que identifica los nodos que son importantes por su capacidad de conectar flujos complejos a través de diferentes modos, no solo por tener muchos vecinos directos.

3.  **Visualización**: Para la representación topológica, hemos utilizado la función nativa `plot_multiplex3D()` de `muxViz`, pasando la lista de grafos y las coordenadas geográficas reales. Esto genera una visualización tridimensional auténtica donde se puede apreciar la estructura de capas (Bus, Metro, BiciMAD, Parking) y su alineación espacial. Adicionalmente, hemos complementado con un dashboard interactivo en `plotly` para la exploración de métricas.

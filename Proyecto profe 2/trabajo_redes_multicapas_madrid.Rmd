...
La visualización comparada de la distribución del **Grado** (número de conexiones) y la **Fuerza** (suma de pesos ponderados por distancia) revela la huella topológica de cada modo. Se observa un contraste radical entre la red de **Autobús (rojo)**, que presenta una distribución aproximadamente normal (campana de Gauss) centrada en grados intermedios, lo que confirma su función de cobertura capilar global, frente a las redes de **Metro (azul)** y **BiciMAD (verde)**, dominadas por una gran barra en grado cero (zonas sin servicio) y una cola de grados altos. Esto último caracteriza a redes selectivas o jerárquicas, donde pocas infraestructuras conectan zonas estratégicas pero dejan gran parte del territorio sin cobertura directa en esa capa específica.
...
1.  **Versatilidad de Grado (Entropía de Shannon)**: Mide el equilibrio de conexiones entre capas. Serán valores entre 0 y 1 dependiendo de lo uniforme que sea la distribución del grado del nodo en las diferentes capas; un valor cercano a 1 indica que el nodo es multimodal (está conectado de forma equilibrada en varios transportes), mientras que un valor cercano a 0 indica que está especializado en un solo modo. Esto es gracias a la entropía de Shannon, que mide la incertidumbre o el desorden de un sistema.
2.  **Multi-Layer PageRank**: Es una métrica de centralidad que mide la influencia o relevancia global de una zona dentro de la red integrada. A diferencia del PageRank convencional, este algoritmo calcula la probabilidad de que un usuario alcance un nodo específico navegando no solo a través de una línea de transporte (movimiento intra-capa), sino también mediante los transbordos entre diferentes modos (movimiento inter-capa). Un nodo con un PageRank alto es un punto de destino o tránsito preferente en el ecosistema total de Madrid.
...
La Figura anterior muestra la relación entre la versatilidad de grado de las zonas y el número medio de viajes por persona a nivel de distrito.
Visualmente, la nube de puntos no presenta una tendencia clara, lo que sugiere una relación débil entre ambas variables. Este resultado se confirma mediante el análisis de correlación de Pearson.
...
Con el análisis conjunto de la versatilidad nodal y la robustez estructural
de la red agregada de Madrid podemos extraer tres conclusiones fundamentales
...
1.  Independencia entre Infraestructura y Demanda Individual. Tal como
    ...
    -   La disposición de los puntos en bandas horizontales discretas 
        revela que la red no es continua, sino jerárquica: la gran mayoría de 
        nodos son unimodales (base del gráfico), mientras que la multimodalidad 
        se da "a saltos" (agregando capas de Metro o BiciMAD).

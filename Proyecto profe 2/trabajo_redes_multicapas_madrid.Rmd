...
datos_plot <- versatilidad_df %>%
  ...
  select(versatilidad_grado, viajes_medios_por_persona, cluster_id)
...
fig_vers_viajes <- plot_ly(
  ...
  marker = list(
    size = 8,
    color = ~versatilidad_grado,
    colorscale = 'Viridis',
    showscale = TRUE,
    colorbar = list(title = "Versatilidad"),
    line = list(color = 'white', width = 0.5)
  ),
  text = ~paste('Zona:', cluster_id, '<br>Viajes/persona:', round(viajes_medios_por_persona, 3), '<br>Versatilidad:', round(versatilidad_grado, 3)),
  hoverinfo = 'text'
) %>%
  layout(
    title = list(
      text = "Relación entre Versatilidad y Viajes por Persona",
      font = list(size = 18, color = '#2c3e50')
    ),
    xaxis = list(
